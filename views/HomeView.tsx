  ...
  return (
    <div style={{ padding:"1.5rem 1.25rem", maxWidth:640, margin:"0 auto", minHeight:"100vh", background:T.bg }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:"1.5rem", flexWrap:"wrap" }}>
        <span style={{ fontWeight:700, fontSize:20, color:T.text, flex:1, letterSpacing:"-0.03em" }}>Projects</span>
        ...
        <button onClick={() => setAddingProject(true)} style={{ padding:"6px 14px", borderRadius:8, border:"none", background:T.accent, color:T.accentText, fontSize:13, fontWeight:500, cursor:"pointer" }}>+ New project</button>
        <div style={{ position:"relative" }} ref={settingsRef}>
          <button onClick={() => setShowSettings(v => !v)} style={{ padding:"6px 12px", borderRadius:8, border:`1px solid ${T.border}`, background:T.surface, color:T.textMuted, fontSize:13, cursor:"pointer" }}>⚙</button>
          {showSettings && (
      ...
      <DragList items={filtered} onReorder={reorderProjects} renderItem={(p: Project) => {
        ...
        return (
          <div style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px", borderRadius:10, border:`1px solid ${T.border}`, background:T.surface, marginBottom:5 }}
            ...
            <DotsMenu options={[{ label:"Delete project", danger:true, action:() => deleteProject(p.id) }]} />
          </div>
        );
      }} />
    </div>
