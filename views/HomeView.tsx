        ...
        <div style={{ position:"relative", flexShrink:0, marginLeft:"auto" }} ref={settingsRef}>
          ...
          {showSettings && (
            <div style={{ position:"absolute", right:0, top:"calc(100% + 6px)", background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:"1.25rem", minWidth:260, zIndex:200, boxShadow:"0 8px 24px rgba(0,0,0,0.08)" }}>
              <div style={{ fontWeight:600, fontSize:13, color:T.text, marginBottom:10 }}>Trade tags</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:12 }}>
                {trades.length === 0 && <div style={{ fontSize:12, color:T.textFaint }}>No trades yet.</div>}
                {trades.map((t: string) => (
                  <span key={t} style={{ display:"inline-flex", alignItems:"center", gap:5, background:T.bg, border:`1px solid ${T.border}`, borderRadius:6, padding:"3px 8px", fontSize:12, color:T.text }}>
                    {t}
                    <span style={{ cursor:"pointer", color:T.textFaint, fontSize:14, lineHeight:1 }} onClick={() => deleteTrade(t)}>×</span>
                  </span>
                ))}
              </div>
        ...
        <div style={{ background:T.surface, border:`1px solid ${T.borderStrong}`, borderRadius:10, padding:"1rem", marginBottom:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
            <input autoFocus value={name} onChange={e => setName(e.target.value)}
              onKeyDown={e => { if (e.key==="Enter") document.getElementById("paddr")?.focus(); if (e.key==="Escape") { setAddingProject(false); setName(""); setAddr(""); } }}
              placeholder="Project name"
              style={{ padding:"9px 12px", borderRadius:8, border:`1px solid ${T.border}`, background:T.bg, fontSize:14, outline:"none", color:T.text }} />
            <input id="paddr" value={addr} onChange={e => setAddr(e.target.value)}
              onKeyDown={e => { if (e.key==="Enter") { addProject(name.trim(), addr.trim()); setName(""); setAddr(""); setAddingProject(false); } if (e.key==="Escape") { setAddingProject(false); setName(""); setAddr(""); } }}
              placeholder="Job address (optional)"
              style={{ padding:"9px 12px", borderRadius:8, border:`1px solid ${T.border}`, background:T.bg, fontSize:14, outline:"none", color:T.text }} />
            <div style={{ display:"flex", gap:6 }}>
              <button onClick={() => { addProject(name.trim(), addr.trim()); setName(""); setAddr(""); setAddingProject(false); }}
                style={{ padding:"6px 14px", borderRadius:8, border:"none", background:T.accent, color:T.accentText, fontSize:13, fontWeight:500, cursor:"pointer" }}>Create</button>
              <button onClick={() => { setAddingProject(false); setName(""); setAddr(""); }}
                style={{ padding:"6px 10px", borderRadius:8, border:`1px solid ${T.border}`, background:"transparent", color:T.textMuted, fontSize:13, cursor:"pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ background:T.surface, border:`1px solid ${T.borderStrong}`, borderRadius:10, padding:"1rem", marginBottom:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
            <input autoFocus value={editName} onChange={e => setEditName(e.target.value)}
              onKeyDown={e => { if (e.key==="Enter") document.getElementById("eaddr")?.focus(); if (e.key==="Escape") setEditingProject(null); }}
              placeholder="Project name"
              style={{ padding:"9px 12px", borderRadius:8, border:`1px solid ${T.border}`, background:T.bg, fontSize:14, outline:"none", color:T.text }} />
            <input id="eaddr" value={editAddr} onChange={e => setEditAddr(e.target.value)}
              onKeyDown={e => { if (e.key==="Enter") saveEdit(); if (e.key==="Escape") setEditingProject(null); }}
              placeholder="Job address (optional)"
              style={{ padding:"9px 12px", borderRadius:8, border:`1px solid ${T.border}`, background:T.bg, fontSize:14, outline:"none", color:T.text }} />
            <div style={{ display:"flex", gap:6 }}>
              <button onClick={saveEdit}
                style={{ padding:"6px 14px", borderRadius:8, border:"none", background:T.accent, color:T.accentText, fontSize:13, fontWeight:500, cursor:"pointer" }}>Save</button>
